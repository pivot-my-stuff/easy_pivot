...
Example: items_and_flags_easy_pivot.sql
...
Demonstrates:
    ...
    * Mixing Boolean and aggregate pivots together
    * Self-documenting Boolean columns

--------------------------------------------------------------*/

SET NOCOUNT ON

DROP TABLE IF EXISTS #items_and_flags

SELECT *
INTO #items_and_flags
FROM
(
    VALUES
        ('Brake Pads',      'Detroit',     125),
        ('Brake Pads',      'Chicago',      80),
        ('Brake Pads',      'Toledo',       95),

        ('Door Handle',     'Detroit',      40),
        ('Door Handle',     'Toledo',       65),

        ('Seat Belt',       'Chicago',     210),
        ('Seat Belt',       'Toledo',      180),

        ('Window Switch',   'Detroit',      55),
        ('Window Switch',   'Chicago',      70),

        ('Fuel Pump',       'Detroit',      30),

        ('Mirror Assembly', 'Chicago',      45),
        ('Mirror Assembly', 'Toledo',       60)
) AS data
(
    ITEM_NAME,
    PLANT,
